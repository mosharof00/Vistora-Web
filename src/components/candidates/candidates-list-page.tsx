...
export async function CandidatesListPage({
  paths,
  searchParams,
}: {
  ...
  if (params.status) {
    query = query.eq("status", params.status as CandidateStatus);
  }
  const searchOr = params.q
    ? ilikeOr(["full_name", "candidate_code", "phone", "email"], params.q)
    : null;
  if (searchOr) {
  ...
  return (
    <div className="space-y-6">
      ...
      <CandidatesTable candidates={rows} basePath={paths.candidates} />
    </div>
  );
}
