"use client";

import Link from "next/link";
import { useRouter } from "next/navigation";
import { useTransition } from "react";
import { Pencil } from "lucide-react";
import { toast } from "sonner";

import { setCurrencyActive } from "@/app/(dashboard)/admin/currencies/actions";
import { StatusBadge } from "@/components/ui/status-badge";
import { buttonVariants } from "@/components/ui/button";
import { cn } from "@/lib/utils";

export type CurrencyListRow = {
  code: string;
  name: string;
  symbol: string;
  decimal_places: number;
  is_active: boolean;
};

export function CurrenciesTable({
  currencies,
}: {
  currencies: CurrencyListRow[];
}) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();

  function toggleActive(row: CurrencyListRow) {
    startTransition(async () => {
      const result = await setCurrencyActive(row.code, !row.is_active);
      if (result?.error) {
        toast.error(result.error);
        return;
      }
      toast.success(
        row.is_active ? "Currency deactivated." : "Currency activated."
      );
      router.refresh();
    });
  }

  if (currencies.length === 0) {
    return (
      <div className="rounded-2xl bg-card px-6 py-12 text-center shadow-sm ring-1 ring-border/60">
        <p className="text-sm text-muted-foreground">No currencies yet.</p>
        <Link
          href="/admin/currencies/new"
          className={cn(buttonVariants(), "mt-4")}
        >
          Add currency
        </Link>
      </div>
    );
  }

  return (
    <div className="overflow-hidden rounded-2xl bg-card shadow-sm ring-1 ring-border/60">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[640px] text-left text-sm">
          <thead>
            <tr className="border-b border-border/70 bg-secondary/40 text-muted-foreground">
              <th className="px-4 py-3 font-medium">Code</th>
              <th className="px-4 py-3 font-medium">Name</th>
              <th className="px-4 py-3 font-medium">Symbol</th>
              <th className="px-4 py-3 font-medium">Decimals</th>
              <th className="px-4 py-3 font-medium">Status</th>
              <th className="px-4 py-3 text-right font-medium">Actions</th>
            </tr>
          </thead>
          <tbody>
            {currencies.map((row) => (
              <tr
                key={row.code}
                className="border-b border-border/40 last:border-0"
              >
                <td className="px-4 py-3">
                  <Link
                    href={`/admin/currencies/${row.code}`}
                    className="font-mono font-medium text-primary hover:underline"
                  >
                    {row.code}
                  </Link>
                </td>
                <td className="px-4 py-3">{row.name}</td>
                <td className="px-4 py-3 text-lg leading-none">{row.symbol}</td>
                <td className="px-4 py-3 tabular-nums text-muted-foreground">
                  {row.decimal_places}
                </td>
                <td className="px-4 py-3">
                  <button
                    type="button"
                    disabled={isPending}
                    onClick={() => toggleActive(row)}
                    title="Click to toggle"
                    className="rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                  >
                    <StatusBadge tone={row.is_active ? "success" : "neutral"}>
                      {row.is_active ? "Active" : "Inactive"}
                    </StatusBadge>
                  </button>
                </td>
                <td className="px-4 py-3 text-right">
                  <Link
                    href={`/admin/currencies/${row.code}`}
                    className={cn(
                      buttonVariants({ variant: "ghost", size: "sm" }),
                      "gap-1.5"
                    )}
                  >
                    <Pencil className="size-3.5" />
                    Edit
                  </Link>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
}
